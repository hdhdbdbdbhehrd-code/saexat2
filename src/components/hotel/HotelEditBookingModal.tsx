...
export function HotelEditBookingModal({ open, onClose, onSave, onUpdate, onDelete, roomNumber, booking }: EditBookingModalProps) {
  ...
  useEffect(() => {
    if (booking) {
      ...
      setCheckIn(today); setCheckOut(tomorrow); setStatus('confirmed');
      setPrice('');
    }
  }, [booking, open]);
              ...
              <div className="rounded-2xl border border-border/70 bg-gradient-to-br from-accent/40 to-transparent p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FieldShell label={t('phone')} icon={<Phone className="h-3 w-3 text-primary/70" />}>
                    <Input value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)} placeholder="+998 90 123 45 67" className="h-9 rounded-lg" />
                  </FieldShell>
                  <FieldShell label={t('whatsapp')} icon={<MessageCircle className="h-3 w-3 text-emerald-500" />}>
                    <Input value={guestWhatsapp} onChange={(e) => setGuestWhatsapp(e.target.value)} placeholder="+998 90 123 45 67" className="h-9 rounded-lg" />
                  </FieldShell>
                  <FieldShell label={t('telegram')} icon={<Send className="h-3 w-3 text-sky-500" />}>
                    <Input value={guestTelegram} onChange={(e) => setGuestTelegram(e.target.value)} placeholder={t('telegramPlaceholder')} className="h-9 rounded-lg" />
                  </FieldShell>
                  <FieldShell label={t('instagram')} icon={<Instagram className="h-3 w-3 text-pink-500" />}>
                    <Input value={guestInstagram} onChange={(e) => setGuestInstagram(e.target.value)} placeholder={t('instagramPlaceholder')} className="h-9 rounded-lg" />
                  </FieldShell>
                  <div className="col-span-2">
                    <FieldShell label={t('email')} icon={<Mail className="h-3 w-3 text-primary/70" />}>
                      <Input type="email" value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} placeholder="email@example.com" className="h-9 rounded-lg" />
                    </FieldShell>
                  </div>
                </div>
              </div>
