...
export function HotelRoomGrid({ bookings, conflictBookings = bookings, onAddBooking, onDeleteBooking, onUpdateBooking, focusBookingId, onFocusConsumed, labelWidth }: RoomGridProps) {
  ...
  const handleHeaderMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const apply = () => {
      ...
      // Clamp scrollLeft so the user can never push past the loaded range
      // while we're suppressing prepend — keeps the drag smooth without jumps.
    ...
    <>
      <div className="relative flex-1 min-h-0 flex flex-col">
        ...
        <div
          ...
          className="timeline-scroll flex-1 overflow-auto select-none"
          style={{ contain: 'layout paint', willChange: 'scroll-position' }}
        >
          <div style={{ minWidth: totalWidth + LABEL_WIDTH }}>
            <div
              className="sticky top-0 z-40 flex cursor-grab bg-card"
                      ...
                      <div key={room.number} className={isExpanded ? 'person-section-expanded' : ''}>
                        <div className={`group/room flex grid-row ${isExpanded ? 'person-section-top-border' : ''}`}
                          ...
                          >
                            ...
                            {(fullRangesByRoom.get(room.number) ?? []).map((rng) => {
                              const left = rng.startHalf * HALF_COL_WIDTH;
                              const width = (rng.endHalf - rng.startHalf) * HALF_COL_WIDTH;
                              const key = `${room.number}:${rng.startHalf}`;
                              const isOpen = openFullKey === key;
                              const rangeBookings = bookings.filter((b) => rng.bookingIds.includes(b.id));
                              return (
                                <Popover
                                  key={key}
                                  open={isOpen}
                                  onOpenChange={(o) => {
                                    setOpenFullKey(o ? key : null);
                                    if (o) {
                                      if (!expandedRooms[room.number]) toggleRoomExpand(room.number);
                                      requestAnimationFrame(() => glowBookings(rng.bookingIds));
                                    }
                                  }}
                                >
                                  <PopoverTrigger asChild>
                                    <button
                                      ...
                                      title={lang === 'ru' ? 'Номер полностью занят' : 'Room fully booked'}
                                    >
                                      <span className="room-full-pill-label">
                                        {lang === 'ru' ? 'Занято' : 'Full'}
                                      </span>
                                    </button>
                                  </PopoverTrigger>
                                  ...
                                  >
                                    <div className="px-2 py-1.5 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
                                      {lang === 'ru' ? `Номер ${room.number} · занятые гости` : `Room ${room.number} · occupied guests`}
                                    </div>
                                    <div className="flex flex-col gap-1 max-h-72 overflow-auto">
                                      ...
                                      {rangeBookings.map((bk) => {
                                        const bedLabel = bk.bedIndex !== undefined
                                          ? `${lang === 'ru' ? 'Гость' : 'Guest'} ${bk.bedIndex + 1}`
                                          : (lang === 'ru' ? 'Весь номер' : 'Whole room');
                                        return (
                                          <button
                                            key={bk.id}
                                            type="button"
                                            ...
                                            className="flex items-center justify-between gap-2 rounded-md px-2 py-1.5 text-left text-xs hover:bg-primary/10 focus:bg-primary/10 outline-none"
                                          >
                                            <span className="flex items-center gap-2 min-w-0">
                                              <span className="inline-block h-2 w-2 rounded-full bg-primary shrink-0" />
                                              <span className="font-semibold truncate">{bk.guestName || (lang === 'ru' ? 'Без имени' : 'No name')}</span>
                                            </span>
                                            <span className="text-[10px] text-muted-foreground shrink-0">{bedLabel}</span>
                                          </button>
                                        );
                                      })}
                                    </div>
          ...
          <div className="relative p-6">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-2xl border border-border bg-background px-3 py-2">
              ...
              <input
                ...
                step="1"
                value={rateDraft}
                onChange={(e) => setRateDraft(e.target.value.slice(0, 28))}
                maxLength={28}
